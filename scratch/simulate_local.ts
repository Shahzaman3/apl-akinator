...
async function simulate() {
  ...
  const player = await Player.findOne({ name: targetName }).lean() as any;
  if (!player) {
  ...
  let probabilitiesMap = new Map<string, number>();
  ...
  let dynamicQuestionData: any = null;
  ...
  while (!isComplete && currentQuestion) {
    ...
    let answer: "YES" | "NO" | "MAYBE" | "DONT_KNOW" = "DONT_KNOW";
    ...
    if (currentQuestion.id < 1000) {
      ...
      answer = (evalVal || "DONT_KNOW") as any;
